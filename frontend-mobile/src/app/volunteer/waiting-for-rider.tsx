  ...
  const createRideAndStartPolling = async (rideJson: string) => {
    try {
      ...
      const response = await fetch(
        'http://127.0.0.1:8000/api/rides/volunteer/create',
        {
          method: 'POST',
          headers: {
      ...
      if (isCancelledRef.current) {
        if (data.volunteer_ride_id) {
          getAuthToken().then((token) => {
            fetch(
              `http://127.0.0.1:8000/api/rides/volunteer/cancel/${data.volunteer_ride_id}`,
              {
                method: 'PATCH',
                headers: { Authorization: `Bearer ${token}` },
              }
  ...
  const startPollingFlow = (rideId: string) => {
    ...
    const autoCancelRideInBackend = async () => {
      try {
        const token = await getAuthToken();
        await fetch(
          `http://127.0.0.1:8000/api/rides/volunteer/cancel/${rideId}`,
          {
            method: 'PATCH',
            headers: { Authorization: `Bearer ${token}` },
          }
    ...
    const checkRideStatus = async () => {
      ...
      try {
        const token = await getAuthToken();
        const response = await fetch(
          `http://127.0.0.1:8000/api/rides/${rideId}/status?ride_type=volunteer`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
    ...
    <ScreenWrapper>
      <View style={styles.container}>
        <Text style={styles.title}>
          {isCreating ? 'מפרסם את הנסיעה...' : 'מחפשים לך נוסע...'}
        </Text>
        <Text style={styles.subtitle}>
          אנא המתן, המערכת מנסה להתאים חולה למסלול שלך ברגעים אלו
        </Text>
      ...
      <Modal visible={showTimeoutMessage} transparent={true} animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.emoji}>❤️</Text>
            <Text style={styles.modalTitle}>תודה רבה!</Text>
            <Text style={styles.modalSubtitle}>
              כל הכבוד על הרצון והלב החם להתנדב! כרגע אין חולה במאגר שזקוק להסעה
              במסלול זה.
            </Text>
            <TouchableOpacity
